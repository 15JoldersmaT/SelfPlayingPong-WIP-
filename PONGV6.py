...
##Vars used to tell if paddle is getting more or less competitive with winner
...
class Ball:
    ...
    def move(self):
        ...
        if gameTimer <= 0:
            ...
            left_scores.append(leftScore)
            right_scores.append(rightScore)

            leftDiffLast = lastRight - lastLeft 
            leftDiff = rightScore - leftScore

            rightDiffLast = lastLeft - lastRight 
...
class paddle:
    ...
    def move(self):
        ...
        leftScoreA = (leftScore - 20000) / 20000
        rightScoreA = (rightScore - 20000) / 20000
        spedd = 0
...
while True:
    ...
    # Append current scores to the lists
    

    # Plot and save the graph every 10 epochs
    ...
    rightScoreT = myFont.render(f"Right Score: {rightScore}", True, (255,255,255))
    display.blit(rightScoreT, (400, 10))  # Position it at the bottom of the screen
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP:
            ...
            if event.key==pygame.K_g:
                plt.figure()
                plt.plot(left_scores, label='Left Paddle Scores')
                plt.plot(right_scores, label='Right Paddle Scores')
                plt.xlabel('Epoch')
                plt.ylabel('Score')
                plt.title('Paddle Scores Over Epochs')
                plt.legend()
                plt.savefig(f'paddle_scores_epoch_{epoch}.png')
                plt.close()
